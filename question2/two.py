...
class L1L2():
    ...
    def L2_loss(self,weights):
        ...
        return -1*(np.sum(np.log(label_probabilities)))+ self.lmda*(np.dot(np.transpose(weights),weights))

    def L1_loss(self,weights):
        preds = logistic_predictions(weights, self.X)
        label_probabilities = (preds ** self.y) * ((1 - preds) **(1 - self.y))
        return -1*(np.sum(np.log(label_probabilities))) + self.lmda* (np.sum(abs(weights)))

    def L1_fit(self,X,y,n_iters=1000,lmda=1,lr=0.01):
        self.lmda=lmda
        self.y=y
        n_samples=len(X)
        ...
        for i in range(n_iters):
            gradient_L1 = grad(self.L1_loss)
            weights -= (lr/n_samples)* (gradient_L1(weights)) 
            # print(weights)
        self.weights= weights


    def L2_fit(self,X,y,n_iters=1000,lmda=1,lr=0.01):
        self.lmda=lmda
        self.y=y
        n_samples=len(X)
        if(self.fit_intercept==True):
        ...
        weights = np.zeros(X_new.shape[1])
        for i in range(n_iters):
            gradient_L2 = grad(self.L2_loss)
            weights -= (lr/n_samples)* (gradient_L2(weights)) 
            # print(weights)
        self.weights= weights
